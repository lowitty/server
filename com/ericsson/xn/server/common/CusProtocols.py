...
class SshCusProtocol(recvline.HistoricRecvLine):
    ...
    def handler_sbc(self, line):
        ...
        if 0 == self.sbcmode:
            protocol_log.info('Mode is 1: ' + 'lhsh ' + self.sbcprops.getProperty('SMN') + self.sbcprops.getProperty(
                'APN') + '00 li_enter PRC')
            if ('lhsh ' + self.sbcprops.getProperty('SMN') + self.sbcprops.getProperty(
                    'APN') + '00 li_enter PRC' == line):
                self.sbcmode = 1
